...
from test_llm import ModelTester
...
class StartScreen(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('Welcome to Robot Vacuum System')
        self.setGeometry(100, 100, 800, 600)

        # Use a main layout that allows precise positioning
        layout = QVBoxLayout()
        layout.setSpacing(0)
        layout.setContentsMargins(0, 0, 0, 0)
        self.setLayout(layout)

        # Main content area (the SASTRA image)
        ...
        # Give the main content more space by setting a larger size policy
        sizePolicy = QSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.label.setSizePolicy(sizePolicy)
        
        # Add the main content to the layout
        layout.addWidget(self.label, 10)  # Give it a stretch factor of 10
        
        # Create a very small container for just the "Press Enter" text
        instruction = QLabel("Press Enter to continue", self)
        instruction.setStyleSheet("color: black; font-size: 12pt; background-color: rgba(240, 240, 240, 200); padding: 5px;")
        instruction.setAlignment(Qt.AlignCenter)
        instruction.setFixedHeight(30)  # Make it much smaller in height
        
        # Add the instruction with minimal space
        layout.addWidget(instruction, 0)  # No stretch (0)
...
# Object Description Model Loader - Fixed Implementation
class ObjectDescriptionModelLoader:
    ...
    def load_model(self, model_dir='saved_models'):
        """Load the object description model using latest_model_paths.json"""
        try:
            # Load the paths to the latest model and tokenizer
            paths_file = os.path.join(model_dir, 'latest_model_paths.json')
            
            if not os.path.exists(paths_file):
                print(f"Model paths file not found: {paths_file}")
                return False
                
            with open(paths_file, 'r') as f:
                paths = json.load(f)
                
            if not os.path.exists(paths['tokenizer_path']):
                print(f"Tokenizer file not found: {paths['tokenizer_path']}")
                return False
                
            if not os.path.exists(paths['model_path']):
                print(f"Model file not found: {paths['model_path']}")
                return False
            ...
            # Verify the checkpoint contains required data
            if 'vocab_size' not in checkpoint:
                print("Checkpoint does not contain vocab_size")
                return False
            ...
            print("Object description model and tokenizer loaded successfully!")
            return True
            
        except Exception as e:
            print(f"Error loading object description model: {str(e)}")
            traceback.print_exc()
            return False
    
    def get_object_description(self, question_text):
        """Get description using the loaded model"""
        if self.model is None or self.tokenizer is None:
            raise ValueError("Object description model or tokenizer not initialized")
        ...
        try:
            with torch.no_grad():
                ...
                # Add start token
                if '<START>' in self.tokenizer.vocab:
                ...
                # Decode response
                response = self.tokenizer.decode(output_tokens[0].tolist())
                return response
        except Exception as e:
            print(f"Error generating object description: {str(e)}")
            traceback.print_exc()
            raise ValueError(f"Failed to generate object description: {str(e)}")
            ...
            try:
                ...
                if features is not None:
                    ...
                    if self.execute_command:
                        ...
                        if self.robot_model:
                            # Use the robot model to execute command
                            robot_response = self.robot_model.execute_command(command_text)
                        else:
                            # If robot model is not available (should not happen with removed fallbacks)
                            robot_response = f"Error: Robot model not available for command: {command_text}"
...
class RobotVoiceCommandSystem(QMainWindow):
    def __init__(self):
        ...
        # Initialize and start the Pygame rendering thread
    ...
    def loadModels(self):
        ...
        try:
            # Check if voice recognition model exists
            ...
            # Load the robot model - no fallback mechanism
            self.robot_model = RobotModelLoader(ROBOT_MODEL_PATH)
            robot_model_loaded = self.robot_model.load_model()
            if not robot_model_loaded:
                self.result_text.setText("Error loading robot command model. Please check the model file.")
                # Continue without disabling so the voice recognition can still work
            
            # Load the object description model - no fallback mechanism
            ...
            if not object_model_loaded:
                self.result_text.setText("Error loading object description model. Please check the model directory.")
            ...
            if robot_model_loaded:
                ...
                status_parts.append("Voice recognition model loaded. Robot command model failed to load.")
            
            if object_model_loaded:
                status_parts.append("Object description model loaded successfully.")
            else:
                status_parts.append("Object description model failed to load.")
    ...
    def process_question(self, question):
        ...
        try:
            ...
            # Check if object model is available
            if not hasattr(self, 'object_model'):
                raise ValueError("Object description model not available")
                
            # Check if model and tokenizer are initialized
            if self.object_model.model is None or self.object_model.tokenizer is None:
                raise ValueError("Object description model not properly initialized")
            ...
            try:
                ...
                print(f"Error from model: {str(model_error)}")
                traceback.print_exc()
                raise ValueError(f"Failed to generate response: {str(model_error)}")
    ...
    def selectFile(self):
        ...
        file_dialog.setNameFilter("Audio Files (*.wav *.mp3 *.m4a *.ogg)")
        ...
        if file_dialog.exec_():
            ...
            if file_paths:
                ...
                try:
                    ...
                    # Update status
                    file_size_kb = os.path.getsize(self.selected_file) / 1024
                    self.status_display.setText(f"Loaded audio file: {os.path.basename(self.selected_file)}\n"
                                              ...
                                              f"Sample rate: {self.sr} Hz | File size: {file_size_kb:.1f} KB\n"
...
def main():
    ...
    if not os.path.exists(ROBOT_MODEL_PATH):
        missing_files.append(ROBOT_MODEL_PATH)
    
    # Check for object description model paths
    model_paths_file = os.path.join('saved_models', 'latest_model_paths.json')
    if not os.path.exists(model_paths_file):
        missing_files.append(model_paths_file)
    else:
        # Verify that the files referenced in the JSON exist
        try:
            with open(model_paths_file, 'r') as f:
                paths = json.load(f)
                
            if 'model_path' in paths and not os.path.exists(paths['model_path']):
                missing_files.append(f"Object model file: {paths['model_path']}")
                
            if 'tokenizer_path' in paths and not os.path.exists(paths['tokenizer_path']):
                missing_files.append(f"Tokenizer file: {paths['tokenizer_path']}")
        except Exception as e:
            print(f"Error checking model paths: {str(e)}")
            missing_files.append(f"Error loading {model_paths_file}: {str(e)}")
    
    if missing_files:
        print(f"Warning: The following required model files are missing: {', '.join(missing_files)}")
...
if __name__ == "__main__":
    try:
        print("Starting Conversational AI based Robot Vacuum Cleaner System...")
        print("Version: 2.0.0 - May 2025")
        print("Using direct model access for Robot LLM and Object Description LLM")
        
        # Verify that the SimpleTokenizer and SimpleObstacleLLM are available
        try:
            print(f"Obstacle LLM module available: {SimpleTokenizer.__name__}, {SimpleObstacleLLM.__name__}")
        except Exception as module_error:
            print(f"Warning: Obstacle LLM module issue: {str(module_error)}")
